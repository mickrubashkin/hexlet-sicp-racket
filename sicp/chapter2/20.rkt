...
; before the last parameter name indicates that, when the procedure is called, 
...
; For instance, given the definition (define (f x y . z) ⟨body⟩)
; the procedure f can be called with two or more arguments.
; If we evaluate (f 1 2 3 4 5 6) then in the body of f, x will be 1, 
; y will be 2, and z will be the list (3 4 5 6).
...
; that takes one or more integers and returns a list of all the arguments 
...
(define (same-parity n . items)
  ...
  (define (iter acc src)
    (if (null? src)
        acc
        (if (same-parity? (car src) n)
            (iter (append acc (list (car src))) (cdr src))
            (iter acc (cdr src)))))
  (iter (list n) items))
